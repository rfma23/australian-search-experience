...
const awDatenspendeBucketName = process.env["DATASPENDE_BUCKET"];
const awDatenspendeTable = process.env["DATASPENDE_TABLE"];
const awDatenspendeUsersTable = process.env["DATASPENDE_USERS_TABLE"];
const awDatenspendeIPCacheTable = process.env["DATASPENDE_IPCACHE_TABLE"];
  ...
  return await dynamoDB
    ...
    .then(async function (data) {
      ...
      data.Items.forEach(function (item) {
        if (
          Math.abs(parseInt(item.createdAt) - current_time) <= CONST_THROTTLE_INTERVAL
        ) {
          users_in_interval += 1;
        }
      });
    ...
    try {
      ...
      if (eventBodyParsed.event == "collection") {
        ...
        async function createUser() {
          ...
          var this_hash = function () {
            var a = [];
            for (var i = 1; i <= 8; i++) {
              a.push(Math.random().toString(36).substr(2, 4));
            }
            return a.join("");
          }.call();
          const current_time = parseInt(Date.now() / 1000);
        ...
        return await IP_check_gate(event, async function () {
          return await IP_log_then(event, async function () {
            ...
            return await ddb
              .describeTable({ TableName: awDatenspendeTable })
              .promise()
              .then(async function (dataAWDatenspendeTable) {
                return await ddb
                  .describeTable({ TableName: awDatenspendeUsersTable })
                  .promise()
                  .then(async function (dataAWDatenspendeUsersTable) {
                    ...
                    return await s3
                      .putObject({
                        ...
                        Body: JSON.stringify({
                          ...
                          number_of_users:
                            dataAWDatenspendeUsersTable["Table"]["ItemCount"],
                          number_of_scrapes:
                            dataAWDatenspendeTable["Table"]["ItemCount"] * 150,
                        }),
                        ContentType: "json",
                      })
                      .promise()
